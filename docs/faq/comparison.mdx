---
title: "Why Not Skills.sh?"
description: "A detailed breakdown of the differences between Skill Harbor and Vercel's skills.sh"
...
Skill Harbor's architecture requires strict separation of concerns into three phases: **Moor** (Fetch) ➔ **Process** (Adapt) ➔ **Berth** (Distribute).
...
### 2. Loss of Cross-Platform Adaptation
...
### Summary Verdict
`skills.sh` is an incredible tool for individual discovery and package management. However, its monolithic approach to fetching and immediately installing makes it incompatible as an underlying dependency for Skill Harbor, which must remain the sovereign "General Contractor" over the fetch, adapt, govern, and distribute lifecycle.
