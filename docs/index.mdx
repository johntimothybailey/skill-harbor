...
![Skill Harbor Hero](/assets/header.png)
...
```mermaid
graph TD
    ProjectNew([Project needs new skill]) -->|⚓ Mooring / dock| B(.harbor/manifest)
    UserExist([User clones/syncs repo]) -->|⚙️ Outfitting / up| C{Processing}
    B -->|Triggers| C
    C -->|Adapt| D[Claude Code]
    C -->|Adapt| E[Cursor]
    C -->|Adapt| F[Antigravity]
    D & E & F -->|🛳️ Berth| G[Agent Context Berth]
    G -->|🕯️ Beacon| H[Fleet Intelligence]
    H -->|📏 Survey| I[Fathom Audit]
```

---

## 🕯️ Entry Points & Discovery

How does an agent know which tool to pull from the berth? Every Skill Harbor setup includes a **Lighthouse Beacon**—a dynamically generated intelligence layer that maps user intent to specific skill triggers.

```mermaid
graph TD
    User([User Prompt]) -->|1. Analysis| Beacon{Fleet Intelligence}
    Beacon -->|2. Matches Trigger| Skill[Target Skill Entry Point]
    Skill -->|3. Validate| Contracts[I/O Contract Check]
    Contracts -->|4. Execute| Action[Agent Action]
    Action -->|5. Result| User
```

This "Zero-Tier" routing ensures your agents don't have to guess. By defining clear `## Trigger` sections in your harbor certified skills, the Beacon teaches the agent exactly when to engage.

---

<CardGroup cols={2}>
  <Card title="⚓ Mooring (Fetching)" icon="anchor">
    Registers and fetches raw skill repositories using `skill-harbor dock` and `up`.
  </Card>
  <Card title="⚙️ Outfitting (Processing)" icon="gear">
    Adapts raw skills into precision-engineered skills for specific agent platforms.
  </Card>
