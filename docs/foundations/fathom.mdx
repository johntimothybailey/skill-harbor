...
<Steps>
  ...
  <Step title="Normalization Formula">
  ...
  <Step title="Context Saturation Math">
...
For the broader mental model behind ghosts, friendly ghosts, and when to use the primary `skill-harbor ghosts` workflow versus `fathom --ghosts`, see [Ghosts](/docs/foundations/ghosts).
...
After the scan, if ghosts are found, Fathom prompts you to **dock them** into your overrides manifest:

```bash
skill-harbor fathom --ghosts

# 👻 Ghost Alert: Found 2 unregistered local skills.
# 🤔 Would you like to dock these to your overrides manifest now? (y/N)
```

Selecting `y` registers each ghost as an override entry in `harbor-manifest.overrides.json`.
